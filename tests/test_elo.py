...
from xpartamupp.elo import (
    ANTI_INFLATION,
    ELO_K_FACTOR_CONSTANT_RATING,
    ELO_SURE_WIN_DIFFERENCE,
    VOLATILITY_CONSTANT,
...
class TestELO(TestCase):
    ...
    @given(
        st.integers(min_value=ELO_K_FACTOR_CONSTANT_RATING),
        st.integers(min_value=-2099, max_value=ELO_SURE_WIN_DIFFERENCE - 1),
        st.integers(),
        st.integers(),
        st.integers(min_value=-1, max_value=1),
    )
    @example(ELO_K_FACTOR_CONSTANT_RATING + 300, 0, 0, 0, 1)
    def test_constant_rating(
        self,
        rating_player1,
        difference_player2,
        played_games_player1,
        ...
        volatility = (
            50.0
            * (min(max(0, played_games_player1), VOLATILITY_CONSTANT) / VOLATILITY_CONSTANT + 0.25)
            / 1.25
        )
        rating_adjustment = (
            difference_player2 + result * ELO_SURE_WIN_DIFFERENCE
    ...
    @given(
        st.integers(min_value=-2199),
        st.integers(min_value=ELO_SURE_WIN_DIFFERENCE),
        st.integers(),
        st.integers(),
    )
    @example(1000, ELO_SURE_WIN_DIFFERENCE, 0, 0)
    def test_sure_loss(
        self, rating_player1, difference_player2, played_games_player1, played_games_player2
    ):
    ...
    def test_minus_2200_bug_workaround(
        self, rating_player1, rating_player2, played_games_player1, played_games_player2, result
    ):
        """Test workaround for -2200 bug."""
        with self.assertRaises(ValueError):
            get_rating_adjustment(
                rating_player1, rating_player2, played_games_player1, played_games_player2, result
            )
        with self.assertRaises(ValueError):
            get_rating_adjustment(
                rating_player2, rating_player1, played_games_player1, played_games_player2, result
            )
